...
class DAN:
    ...
    def F_attention(self, inputs):
        sp_avg_pool = TimeDistributed(
            Lambda(lambda x: K.mean(x, axis=-1, keepdims=True))
        )(inputs)
        sp_max_pool = TimeDistributed(
    ...
    def C_attention(self, inputx, n, ratio=8):
        ...
        channel_attention_feature = K.permute_dimensions(
            channel_attention_feature, (0, 1, 3, 2)
        )
        inputx = K.permute_dimensions(inputx, (0, 1, 3, 2))
        # performs attention mechanism across channel dimension
        channel_attention_feature = self.attention_layer(
            channel_attention_feature, inputx
        )
    ...
    def MHDAN(self, inputx, neurons_1, neurons_2, n, h):
        ...
        F1, F2, F3 = (
            self.F_attention(inputx[:, :, : d2 // h, : d3 // h]),
            self.F_attention(
                inputx[:, :, d2 // h : d2 // h + d2 // h, d3 // h : d3 // h + d3 // h]
            ),
            self.F_attention(inputx[:, :, d2 // h + d2 // h :, d3 // h + d3 // h :]),
        )
        # combine the heads across spectral dimension
        F = Concatenate(axis=2)([F1, F2, F3])
        # attention across channels/electrodes
        C1, C2, C3 = (
            self.C_attention(inputx[:, :, : d2 // h, : d3 // h], n, ratio=8),
            self.C_attention(
                inputx[:, :, d2 // h : d2 // h + d2 // h, d3 // h : d3 // h + d3 // h],
                n,
                ratio=8,
            ...
            self.C_attention(
                inputx[:, :, d2 // h + d2 // h :, d3 // h + d3 // h :], n, ratio=8
            ),
        )
